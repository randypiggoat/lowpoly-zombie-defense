...
export type TargetMode = "first" | "last" | "strongest" | "weakest";
...
export class Game {
  ...
  tick(dtRaw: number) {
    ...
    for (const t of s.towers) {
      ...
      let bestScore = t.targetMode === "first" ? -Infinity : Infinity;
      for (const z of s.zombies) {
        if (z.dead) continue;
        const d = Math.hypot(z.x - t.x, z.z - t.z);
        if (d > range) continue;

        // "first" = furthest along the path, "last" = closest to spawn.
        // Strongest/weakest compare current HP. A small distance tie-breaker
        // keeps targeting stable when two zombies have identical HP.
        let score: number;
        switch (t.targetMode) {
          case "last":
            score = z.dist;
