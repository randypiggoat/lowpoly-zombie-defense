            ...
            <div className="mt-2 rounded-xl bg-black/25 p-2">
              ...
              <div className="mt-1 grid grid-cols-4 gap-1">
                {TARGET_MODES.map((mode) => (
                  <button
                    key={mode.id}
                    onClick={() => game.setTargetMode(tower.id, mode.id)}
                    className={`rounded-lg px-1.5 py-2 text-[10px] font-semibold transition active:scale-[0.98] ${
                      tower.targetMode === mode.id
                        ? "bg-accent text-accent-foreground"
                        : "bg-black/30 text-panel-muted"
                    }`}
                  >
                    {mode.label}
                  </button>
